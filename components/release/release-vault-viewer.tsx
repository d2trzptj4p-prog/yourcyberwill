      ...
      <div className="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl">
        ...
        <div className="flex gap-2 items-center mt-3">
          <Input
            readOnly
            value={value}
            className="flex-1 rounded-lg border border-zinc-300 bg-zinc-50 px-3 py-2 font-mono text-sm text-zinc-900 focus-visible:ring-0 focus-visible:ring-offset-0"
            onClick={(e) => (e.target as HTMLInputElement).select()}
          />
          <Button
            type="button"
            onClick={handleCopy}
            className="rounded-lg bg-zinc-900 px-4 py-2 text-sm font-medium text-white hover:bg-zinc-800 min-w-[75px]"
          >
            {copied ? "Copied" : "Copy"}
          </Button>
        </div>
        <div className="mt-5 flex justify-end">
          <Button
            type="button"
            onClick={onClose}
            className="rounded-full border border-zinc-300 px-4 py-1.5 text-sm text-zinc-600 bg-white hover:bg-zinc-50"
          >
            Close
          </Button>
        </div>
...
export function ReleaseVaultViewer({ token }: { token: string }) {
  ...
  if (loading) {
    return (
      <div className="py-12 text-center bg-white min-h-screen">
        <p className="text-base text-zinc-600 animate-pulse font-medium">
  ...
  return (
    <div className="max-w-4xl mx-auto my-6 px-4 space-y-8 bg-white text-zinc-900 antialiased selection:bg-zinc-100 print:my-0 print:px-0">
      
      {/* Header Info */}
      <header className="border-b border-zinc-200 pb-6 print:pb-4">
        <span className="text-xs font-semibold uppercase tracking-wider text-zinc-500 bg-zinc-100 px-2.5 py-1 rounded-md print:border print:border-zinc-300 print:bg-transparent">
        ...
        <div className="mt-4 flex flex-wrap gap-3 items-center justify-between text-xs text-zinc-500 bg-zinc-50 border border-zinc-100 rounded-xl p-3 print:hidden">
      ...
      <div className="flex border-b border-zinc-200 gap-2 print:hidden">
        <button
          onClick={() => setActiveTab("passwords")}
          className={`pb-3 px-2 text-sm font-medium border-b-2 transition-all ${
            activeTab === "passwords" ? "border-zinc-900 text-zinc-900" : "border-transparent text-zinc-500 hover:text-zinc-900"
          }`}
        >
        ...
        <button
          ...
          className={`pb-3 px-2 text-sm font-medium border-b-2 transition-all ${
            activeTab === "notes" ? "border-zinc-900 text-zinc-900" : "border-transparent text-zinc-500 hover:text-zinc-900"
          }`}
        >
        ...
        <button
          ...
          className={`pb-3 px-2 text-sm font-medium border-b-2 transition-all ${
            activeTab === "files" ? "border-zinc-900 text-zinc-900" : "border-transparent text-zinc-500 hover:text-zinc-900"
          }`}
        >
      ...
      <div className={`${activeTab === "passwords" ? "block" : "hidden"} print:block print:space-y-4`}>
        <h2 className="text-lg font-medium text-zinc-900 hidden print:block print:mb-2 print:border-b print:pb-1">
          1. Account Passwords & Credentials
        </h2>
        {passwords.length === 0 ? (
          <p className="text-sm text-zinc-400 italic py-2">No account passwords left in this directory.</p>
        ) : (
          <div className="overflow-x-auto border border-zinc-200 rounded-xl print:border-zinc-300">
            <table className="w-full min-w-[32rem] table-fixed text-left text-sm print:min-w-0">
              <thead>
                <tr className="border-b border-zinc-200 bg-zinc-50 text-zinc-600 print:bg-zinc-100">
              ...
              <tbody className="divide-y divide-zinc-100 print:divide-zinc-200">
                {passwords.map((item) => (
                  <tr key={item.id} className="hover:bg-zinc-50/50 print:hover:bg-transparent page-break-inside-avoid">
                    {/* Truncated Account Type Button */}
                    <td className="py-3 px-4 font-medium text-zinc-900 max-w-[150px]">
                      <button
                        type="button"
                    ...
                    <td className="py-3 px-4 font-mono text-xs text-zinc-600 max-w-[200px]">
                      <button
                        type="button"
                    ...
                    <td className="py-3 px-4 font-mono text-xs text-zinc-900 max-w-[250px]">
                      <div className="flex items-center justify-between gap-2 max-w-full print:block">
                        <div className="truncate flex-1 print:hidden">
                          {hiddenPasswords.has(item.id) ? (
                            <span className="text-zinc-400 tracking-wider">••••••••</span>
                          ) : (
                            <button
                              type="button"
                        ...
                        <button
                          ...
                          className="text-zinc-500 hover:text-zinc-900 underline text-xs ml-2 shrink-0 print:hidden"
                        >
      ...
      <div className={`${activeTab === "notes" ? "block" : "hidden"} print:block print:space-y-4 print:pt-6`}>
        <h2 className="text-lg font-medium text-zinc-900 hidden print:block print:mb-2 print:border-b print:pb-1 print:page-break-before-always">
          2. Letters & Legacy Instructions
        </h2>
        {notes.length === 0 ? (
          <p className="text-sm text-zinc-400 italic py-2 print:block">No custom text records provided.</p>
        ) : (
          <ul className="space-y-3 print:space-y-6">
            {notes.map((note) => {
              const open = expandedNoteId === note.id;
              return (
                <li key={note.id} className="rounded-xl border border-zinc-200 overflow-hidden bg-white shadow-sm print:shadow-none print:border-zinc-300 print:break-inside-avoid">
                  <Button
                    type="button"
                    variant="ghost"
                    className="flex w-full items-center justify-between px-5 py-4 text-left text-base font-medium text-zinc-900 bg-zinc-50/50 hover:bg-zinc-50 print:bg-zinc-100 print:py-2 print:px-4"
                    onClick={() => setExpandedNoteId(open ? null : note.id)}
                  >
                    <span>{note.title}</span>
                    <span className="text-zinc-400 text-lg print:hidden">{open ? "−" : "+"}</span>
                  </Button>
                  <div className={`${open ? "block" : "hidden"} border-t border-zinc-100 px-5 py-4 bg-white text-zinc-800 leading-relaxed max-w-none prose prose-zinc print:block print:border-zinc-200 print:px-4 print:py-3`}>
      ...
      <div className={`${activeTab === "files" ? "block" : "hidden"} print:block print:space-y-4 print:pt-6`}>
        ...
        {files.length === 0 ? (
          <p className="text-sm text-zinc-400 italic py-2 print:block">No legacy documents uploaded into this segment.</p>
        ) : (
          <ul className="space-y-2">
            {files.map((file) => {
              const open = previewFileId === file.id;
              return (
                <li key={file.id} className="rounded-xl border border-zinc-200 overflow-hidden bg-white shadow-sm print:shadow-none print:border-zinc-300 print:break-inside-avoid">
                  <div className="flex items-center justify-between gap-4 px-5 py-4 bg-zinc-50/50 print:bg-zinc-50 print:py-2 print:px-4">
                    ...
                    <Button
                      type="button"
                      variant="outline"
                      className="text-xs text-zinc-700 bg-white border-zinc-300 px-3 py-1.5 rounded-full hover:bg-zinc-50 print:hidden"
                  ...
                  {open && (
                    <div className="border-t border-zinc-100 p-4 bg-zinc-50/20 flex justify-center print:hidden">
